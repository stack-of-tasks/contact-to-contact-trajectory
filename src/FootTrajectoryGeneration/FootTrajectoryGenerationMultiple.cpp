...
FootTrajectoryGenerationMultiple::FootTrajectoryGenerationMultiple(FootSpecifications *aFoot)
{
  m_Foot = aFoot;
  m_Sensitivity=0.0;
}
...
void FootTrajectoryGenerationMultiple::SetNumberOfIntervals(int lNumberOfIntervals)
{
  ...
  for(unsigned int i=0;i<m_SetOfFootTrajectoryGenerationObjects.size();i++)
    {
      m_SetOfFootTrajectoryGenerationObjects[i] = new FootTrajectoryGenerationStandard(m_Foot);
...
{
  return (int)m_SetOfFootTrajectoryGenerationObjects.size();
}
